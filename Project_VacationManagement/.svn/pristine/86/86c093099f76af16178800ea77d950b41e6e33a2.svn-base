$(function() {
	...
	$.ajax({
		...
		success: function(data) {
			if (data.baseResult.returnCode == "S20001") {
				// 올해 사용한 휴가 수 계산 - header.jsp에 연결하기 위해 여기로 옮김
				console.log("세션체크 성공;ㅅ;");
				$.ajax({
					...
					success: function(data) {
						if (data.baseResult.returnCode == "S00000") {
							$("#countVacation").text(data.returnData);
						} else if(data.baseResult.returnCode == "F11111") {
							console.log("에러");
							location.href = "/main/errorPage";
						} else {
							console.log(data.baseResult.returnDesc);
						}
					},
	...
	function updatePwProcess() {
		...
		$.ajax({
			...
			dataType: "json",
			success: function(data) {
				if (data.baseResult.returnCode == "S00004") {
					location.href = "/member/logout";
				} else {
					pwStatus(data.baseResult.returnDesc);
				}
			},
			error: function(xhr, status) {
				document.write(xhr + " : " + status);
