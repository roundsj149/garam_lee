	...
	function searchOvertime(pageInfo) {
		...
		$.ajax({
			...
			type : 'POST', 
			data : {"searchWord":searchWord, "selectYear":$("select").children("option:selected").val()},
			dataType:"json",
			success : function(data) {
				if(data.result.returnCode == "SUCCESS") {
					allCount = data.cntAllOvertime;
					console.log("초과근무 게시물 수: "+allCount);
				} else {
				...
				$.ajax({
					...
					type : 'POST', 
					data : {"searchWord":searchWord, "selectYear":$("select").children("option:selected").val(), "currPage":currPage},
					dataType:"json",
					success : function(data) {
						console.log("조회 시작");
