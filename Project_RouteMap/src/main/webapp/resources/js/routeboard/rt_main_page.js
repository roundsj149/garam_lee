...
function checkLogin(session) {
   ...
   if(sessionCheck == false) {
      alert('로그인 후 이용해주세요');
      location.href = "../member/login_page.do?uri=routeboard/rt_main_page.do";
      return false;
   } else {
      location.href = 'rt_write_content_page.do';
      return true;
   }
}
...
function allList(search_word, pageInfo) {
   ...
   $.ajax({
       url: "/routeboard/rt_main_process.do",
       type:"get",
       ...
       success:function(result){
         ...
         if(result == "") {
            if(search_word == undefined) {
               var noResult = '<div>게시물이 존재하지 않습니다.<div>';
       ...
       for(data of result) {
          ...
          str += '<div class="row">';
           str += '<div class="col">';
           str += '<div class="row no-gutters border-bottom rounded overflow-hidden flex-md-row h-md-250 position-relative">';
          str += '<div class="col-auto d-none d-lg-block">';
          if(data.locationboardUploadFileVo != null) {
             str += '<img width="200" height="250" src="../../routemap/storage/'+data.locationboardUploadFileVo.locationboard_file_link_path+'" alt="이미지를 불러올 수 없습니다.">';
          }
         str += '</div>';
         str += '<div class="col p-4 flex-column position-static">';
         // str += '<strong class="d-inline-block mb-2
			// text-primary">World</strong>';
         str += '<h5 class="list-title-overflow"><a class="custom-link" href="/routeboard/rt_read_content_page.do?routeboard_no='+data.routeboardVo.routeboard_no+'">'+data.routeboardVo.routeboard_title+'</a></h5>';
         str += '<p class="list-box-overflow">'+data.routeboardVo.routeboard_content+'</p>';
         // str += '<div class="mb-1 text-muted">Nov 12</div>';
         // str += '<p class="card-text mb-auto"></p>';
         

         str += '<span style="float:right">';
         str += '<i id="'+data.routeboardVo.routeboard_no+'" class="far fa-heart fa-lg" style="color: #f06595;cursor: pointer" onclick="like_func(this.id)"></i><span style="color:#959595">&middot;</span>';
         str += '<i class="'+far+'" id="favorite_'+data.routeboardVo.routeboard_no+'"style="color: green;cursor: pointer" onclick ="favorite_check(this.id)"></i><span style="color:#959595">&middot;</span>';
         var milliseconds = data.routeboardVo.routeboard_writedate;
...
function allPageNavigation(count, currPageInfo, search_word) {
   ...
   $('#page_navi').append('<li id="beginPage" class="page-item"><a class="page-link" onclick="allList('+search_word+','+(endPage+1)+')"aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>');
   if(beginPage == 1) {
...
function provinceList(pageInfo) {
   ...
   for(var i=0; i<provinceCheckboxNum.length; i++) {
      ...
      $.ajax({
         url: "/routeboard/rt_main_process.do?currPage="+currPage,
         type:"get",
   ...
   function provincePageNavigation(count, currPageInfo) {
      ...
      $('#page_navi').append('<li id="beginPage" class="page-item"><a class="page-link" onclick="provinceList('+search_word+','+(endPage+1)+')"aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>');
      if(beginPage == 1) {
         $('#beginPage').addClass("disabled");
      }
      for(var i=beginPage; i<= endPage; i++) {
   ...
   function searchList(pageInfo) {
      ...
      $.ajax({
         url: "/routeboard/rt_main_process.do?currPage="+currPage,
         type:"get",
         data: {"provinceCheckboxArr": provinceCheckboxArr, "categoryNum":categoryNum, "costCheckboxArr": costCheckboxArr, "timeCheckboxArr": timeCheckboxArr, "alignNum":alignNum, "searchWord":searchWord, "currPage":currPage},
         dataType:"json",
         success:function(result){
   ...
   function searchPageNavigation(count, currPageInfo) {
      ...
      $('#page_navi').append('<li id="beginPage" class="page-item"><a class="page-link" onclick="searchList('+search_word+','+(endPage+1)+')"aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>');
      if(beginPage == 1) {
         $('#beginPage').addClass("disabled");
      }
      for(var i=beginPage; i<= endPage; i++) {
   ...
   if(session_check == "false") {
      ...
      form.append("routeboard_no",splitId[1])
       ...
       $.ajax({
         ...
         cache : false, // 캐시 여부
         contentType : false, // 요청 컨텐트 타입 "application/x-www-form-urlencoded;
								// charset=UTF-8"
