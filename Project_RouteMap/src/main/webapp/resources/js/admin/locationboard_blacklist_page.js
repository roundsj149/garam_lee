...
setInterval(function() {
	...
	if(totalCount < newCount) {
		noticeLocationPopupInit();
		locationBlacklist(currPage);
		totalCount = newCount;
	}
}, 2000);

//새 신고 있을 때 알림 띄우기
var noticeLocationPopupTimer = setTimeout(function(){}, 1);

function noticeLocationPopupInit() {
 ...
 element.html('<div class="col"><img src="/resources/img/map.png" width="50px" height="60px" style="float:left"></div><div class="col">새로운  <strong>여행지/맛집 게시물 신고</strong>가 접수되었습니다</div>');
 var link = '<a href="/admin/locationboard_blacklist_page.do"></a>';
 element.wrap(link);
 
 // Clear Animation
 element.stop();
 clearTimeout(noticeLocationPopupTimer);
 // Start Animation
 element.animate({'bottom':'0px'}, 'slow', function() {
	 noticeLocationPopupTimer = setTimeout(function() {
		 element.animate({'bottom':'-200px'}, 'slow');
	 }, 10000);
 });
};

// 1-1. 여행지/맛집 게시물 신고 리스트
function locationBlacklist(curr_page) {
	...
	$.ajax({
		type: "POST",
		url : "/admin/location_blacklist_process.do",
...
//			}
			...
			for(data of result) {
				...
				str+= '<td>'+data.locationboard_no+'</td>';
				str+= '<td>'+data.locationboard_title+'</td>';
...
// 1-2. 여행지/맛집 게시물 신고 리스트 페이징
function pageNavigation(count, currPageInfo) {
	
	$('#location_blacklist_pagination').empty();
	...
	$('#location_blacklist_pagination').append('<li id="beginPage" class="page-item"><a class="page-link" onclick="locationBlacklist('+(beginPage-1)+')"aria-label="Previous"><i class="fas fa-angle-left"></i></a></li>');
	if(beginPage == 1) {
		$('#beginPage').addClass("disabled");
	}
	for(var i=beginPage; i<= endPage; i++) {
		$('#location_blacklist_pagination').append('<li id="currPage_'+i+'" class="page-item"><a class="page-link" href="javascript:void(0);" onclick="locationBlacklist('+i+')">'+i+'</a></li>');
		if(i == currPage) {
			$('#currPage_'+i+'').addClass("active");
		}
	}
	$('#location_blacklist_pagination').append('<li id="endPage" class="page-item"><a class="page-link" onclick="locationBlacklist('+(endPage+1)+')"aria-label="Next"><i class="fas fa-angle-right"></i></a></li>');
...
function changeStatus(id) {
	...
	$.ajax({
		type: "POST",
		url : "/admin/location_blacklist_check.do",
