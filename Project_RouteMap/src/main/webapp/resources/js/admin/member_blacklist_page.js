...
setInterval(function() {
	...
	if(totalCount < newCount) {
		noticeMemberPopupInit();
		memberBlacklist(currPage);
		totalCount = newCount;
	}
}, 2000);

// 새 신고 있을 때 알림 띄우기
var noticeMemberPopupTimer = setTimeout(function(){}, 1);

function noticeMemberPopupInit() {
 ...
 element.html('<div class="col"><img src="/resources/img/map.png" width="50px" height="60px" style="float:left"></div><div class="col">새로운  <strong>사용자 신고</strong>가 접수되었습니다</div>');
 var link = '<a href="/admin/member_blacklist_page.do"></a>';
 element.wrap(link);
 
 // Clear Animation
 element.stop();
 clearTimeout(noticeMemberPopupTimer);
 // Start Animation
 element.animate({'bottom':'0px'}, 'slow', function() {
	 noticeMemberPopupTimer = setTimeout(function() {
...
// 1-1. 사용자 신고 리스트
function memberBlacklist(curr_page) {
	...
	$.ajax({
		type: "POST",
		url : "/admin/member_blacklist_process.do",
...
//			}
			...
			for(data of result) {
				...
				str+= '<td>'+data.reporting_member_id+'</td>';
				str+= '<td>'+data.reported_member_id+'</td>';
...
function pageNavigation(count, currPageInfo) {
	...
	$('#member_blacklist_pagination').empty();
	...
	$('#member_blacklist_pagination').append('<li id="beginPage" class="page-item"><a class="page-link" onclick="memberBlacklist('+(beginPage-1)+')"aria-label="Previous"><i class="fas fa-angle-left"></i></a></li>');
	if(beginPage == 1) {
		$('#beginPage').addClass("disabled");
	}
	for(var i=beginPage; i<= endPage; i++) {
		$('#member_blacklist_pagination').append('<li id="currPage_'+i+'" class="page-item"><a class="page-link" href="javascript:void(0);" onclick="memberBlacklist('+i+')">'+i+'</a></li>');
		if(i == currPage) {
			$('#currPage_'+i+'').addClass("active");
		}
	}
	$('#member_blacklist_pagination').append('<li id="endPage" class="page-item"><a class="page-link" onclick="memberBlacklist('+(endPage+1)+')"aria-label="Next"><i class="fas fa-angle-right"></i></a></li>');
...
function changeStatus(id) {
	...
	if($("#"+id+"").text() == '확인') {
		...
		$("#"+id+"").text('확인');
		status = 'Y';
		//alert('미확인->확인'+report_no[1]);
		
	}
	
	$.ajax({
		type: "POST",
		url : "/admin/member_blacklist_check.do",
